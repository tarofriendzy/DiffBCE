...
class Pooler(nn.Module):
    ...
    def forward(self, attention_mask, outputs):
        ...
        if self.pooler_type in ['cls_before_pooler', 'cls']:
            ...
            pooled_result = ((first_hidden + last_hidden) / 2.0 * attention_mask.unsqueeze(-1)) \
                                .sum(1) / attention_mask.sum(-1).unsqueeze(-1)
            return pooled_result
        elif self.pooler_type == "avg_top2":
            second_last_hidden = hidden_states[-2]
            last_hidden = hidden_states[-1]
            pooled_result = ((last_hidden + second_last_hidden) / 2.0 * attention_mask.unsqueeze(-1))\
                                .sum(1) / attention_mask.sum(-1).unsqueeze(-1)
            return pooled_result
        else:
...
def cl_init(cls, config):
    ...
    cls.init_weights()
    cls.generator = transformers.DistilBertForMaskedLM.from_pretrained(
...
def cl_forward(cls, encoder, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None,
               ...
               return_dict=None, mlm_input_ids=None, mlm_labels=None, cls_token=101, ):
    """
    Conditional Difference Prediction
    """
    return_dict = return_dict if return_dict is not None else cls.config.use_return_dict

    ori_input_ids = input_ids  # Tensor: (64, 2, 32)
    batch_size = input_ids.size(0)  # int: 64
    ...
    input_ids = input_ids.view((-1, input_ids.size(-1)))  # (batch * num_sent, len), Tensor: (128, 32)
    attention_mask = attention_mask.view((-1, attention_mask.size(-1)))  # (batch * num_sent, len), Tensor: (128, 32)
    if token_type_ids is not None:
        token_type_ids = token_type_ids.view((-1, token_type_ids.size(-1)))  # (bs * num_sent, len), Tensor: (128, 32)

    # Get raw embeddings
    outputs = encoder(
        input_ids,  # Tensor: (128, 32)
        attention_mask=attention_mask,  # Tensor: (128, 32)
        token_type_ids=token_type_ids,  # Tensor: (128, 32)
    ...
    # Pooling
    pooler_output = cls.pooler(attention_mask, outputs)  # Tensor: (128, 768)
    pooler_output = pooler_output.view((batch_size, num_sent, pooler_output.size(-1)))  # (bs, num_sent, hidden), Tensor: (64, 2, 768)
    ...
    if mlm_input_ids is not None:
        mlm_input_ids = mlm_input_ids.view((-1, mlm_input_ids.size(-1)))  # (128, 32)
        with torch.no_grad():
            g_pred = cls.generator(mlm_input_ids, attention_mask)[0].argmax(-1)  # (128, 32)
        g_pred[:, 0] = cls_token
        replaced = (g_pred != input_ids) * attention_mask  # (128, 32)
        e_inputs = g_pred * attention_mask  # (128, 32)
    ...
    if dist.is_initialized() and cls.training:
        ...
        # Since all gather results do not have gradients, we replace the
    ...
    if mlm_outputs is not None and mlm_labels is not None:
        ...
        e_labels = replaced.view(-1, replaced.size(-1))  # (128, 32)

        # prediction_scores = cls.lm_head(mlm_outputs.last_hidden_state)
        prediction_scores = cls.electra_head(mlm_outputs.last_hidden_state)  # (128, 32, 2)
        rep = (e_labels == 1) * attention_mask
        fix = (e_labels == 0) * attention_mask
        prediction = prediction_scores.argmax(-1)  # (128, 32)
        ...
        tmp = prediction_scores.view(-1, 2)
        tmp = e_labels.view(-1)
