...
class CLTrainer(Trainer):
    ...
    def evaluate(self, eval_dataset: Optional[Dataset] = None, ignore_keys: Optional[List[str]] = None,
                 metric_key_prefix: str = "eval", eval_senteval_transfer: bool = False, ) -> Dict[str, float]:
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        tr_others = {'electra_acc': torch.tensor(0.0).to(self.args.device),
                     'electra_rep_acc': torch.tensor(0.0).to(self.args.device),
                     'electra_fix_acc': torch.tensor(0.0).to(self.args.device)}
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    self._maybe_log_save_evaluate(tr_loss, model, trial, epoch, tr_pos_sim=tr_pos_sim,
                                                  tr_neg_sim=tr_neg_sim, tr_others=tr_others)

                if self.control.should_epoch_stop or self.control.should_training_stop:
                    break

            self.control = self.callback_handler.on_epoch_end(self.args, self.state, self.control)
            self._maybe_log_save_evaluate(tr_loss, model, trial, epoch, tr_pos_sim=tr_pos_sim, tr_neg_sim=tr_neg_sim,
                                          tr_others=tr_others)
