...
class ModelArguments:
    ...
    generator_name: Optional[str] = field(
        default=None,
        metadata={"help": "The model checkpoint for weights of the generator model."},
    )
    ...
    batchnorm: bool = field(
        default=False,
        metadata={"help": "Whether to use two-layer mlp for the pooler."}
    )
    lambda_weight: float = field(
        default=0.1,
        metadata={"help": "Weight for lambda."}
    )
    ...
    before_mlp: bool = field(
        default=False,
        metadata={"help": "Use the embedding before MLP for the conditional electra task"}
    )
...
def main():
    ...
    if extension == "csv":
        ...
        datasets = load_dataset(extension, data_files=data_files) # DatasetDict, shape = {'train': (1000000, 1)}
    ...
    column_names = datasets["train"].column_names  # value: ['text']
    sent2_cname = None
    ...
    def prepare_features(examples):
        ...
        padding = longest (default)
          If no sentence in the batch exceed the max length,
          then use the max sentence length in the batch,
          otherwise use the max sentence length in the argument and truncate those that exceed the max length.
    ...
    class OurDataCollatorWithPadding:
        ...
        def mask_tokens( self, inputs: torch.Tensor, special_tokens_mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
            ...
            probability_matrix = torch.full(
                labels.shape,
                self.mlm_probability if self.tokenizer.custom_mlm_prob is None else self.tokenizer.custom_mlm_prob)
